...
import { MOB_SECTIONS } from '@/lib/constants/checklists';
...
export default function MobPage() {
  ...
  const handleToggle = async (id: string, completed: boolean) => {
    ...
    const { error } = await supabase.from('mob_checklist').update({ completed, completed_at: completed ? new Date().toISOString() : null }).eq('id', id);
    if (error) {
  ...
  const handleNotes = async (id: string, notes: string) => {
    ...
    const { error } = await supabase.from('mob_checklist').update({ notes }).eq('id', id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Mobilization Checklist</h1>
          <p className="text-sm text-gray-500">48 items across 6 sections</p>
        </div>
        <div className="w-48">
