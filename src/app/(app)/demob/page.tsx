...
import { DEMOB_SECTIONS } from '@/lib/constants/checklists';
...
export default function DemobPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('demob_checklist')
      .select('*')
      .eq('project_id', currentProject.id)
      .order('item_number', { ascending: true });
    if (error) { toast('error', 'Failed to load checklist: ' + error.message); }
    else { setItems(data || []); }
    setLoading(false);
  ...
  const handleToggle = async (id: string, completed: boolean) => {
    ...
    const { error } = await supabase.from('demob_checklist').update({ completed, completed_at: completed ? new Date().toISOString() : null }).eq('id', id);
    if (error) {
  ...
  const handleNotes = async (id: string, notes: string) => {
    ...
    const { error } = await supabase.from('demob_checklist').update({ notes }).eq('id', id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Demobilization Checklist</h1>
          <p className="text-sm text-gray-500">35 items across 5 sections with priority levels</p>
        </div>
        <div className="w-48">
